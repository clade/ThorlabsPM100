...
def _generic_command(cmd_name, doc=None):
    ...
    def commande(self):
        ...
        return None
    commande.__name__ = "_%s"%(cmd_name.replace(':','_').lower())
    commande.__doc__ = commande.__name__ if doc is None else doc
    return commande
...
class GenericGetCommandClass(GenericCommandClass):
    @classmethod
    def to_dict(cls, name):
        get_cmd = _generic_get_command(cls.cmd, cls.out_conversion, doc=cls.__doc__)
...
class GenericSetCommandClass(GenericCommandClass):
    ...
    def to_dict(cls, name):
        ...
        return {name:property(lambda self:None, set_cmd,doc=cls._get_the_doc()), 
...
#            out += "**Property value (write-only) :** "+','.join(args)+"\n\n"
        out += "**Property value (write-only) :** "+','.join(map(str, args[0].list_test_value))+"\n\n"
        out += "**Initial SCPI command :** {0}\n\n".format(cls.cmd)
        return _make_doc(out, title)
...
#            out += "**Property value :** "+','.join(args)+"\n\n"
        args = cls.get_argument_list()
        out += "**Property value :** "+','.join(map(str, args[0].list_test_value))+"\n\n"
        out += "**Initial SCPI command :** {0}\n\n".format(cls.cmd)
        return _make_doc(out, title)       
...
class IndexedGroup(Group):
    ...
    def __getitem__(self, i):
         ...
         return out        
    def _get_cmd_name(self, cmd_name):
        new_cmd = self._parent._get_cmd_name(cmd_name).replace(self.var, str(self._item))
        return new_cmd
